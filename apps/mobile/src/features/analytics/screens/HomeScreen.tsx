    ...
    <Screen scroll contentStyle={styles.screen}>
      ...
      {transactions.slice(0, 3).map(item => (
        <View key={item.id} style={styles.row}>
          <View><Text style={styles.rowTitle}>{item.note || (item.type === 'income' ? '收入' : '支出')}</Text><Text style={styles.caption}>{item.occurredAt.slice(0, 10)}</Text></View>
          <Text style={item.type === 'income' ? styles.income : styles.expense}>{item.type === 'income' ? '+' : '-'} {money(item.amountCent)}</Text>
        </View>
      ))}
...
const styles = StyleSheet.create({
  ...
  actionPlus: { color: '#FFFFFF', fontSize: 38, lineHeight: 42 },
